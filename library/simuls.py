...
def generate_covariances_and_a(n_matrices, n_channels, n_sources, rank=None,
...
def generate_covariances(n_matrices, A, n_channels, n_sources, rank=None,
                         distance_projs=0., sigma=0., 
                         noise_A=0., f_p='log', sigma_noise=1., rng=0):
    rng = check_random_state(rng)
    project = rank is not None
    ...
    for i in range(n_matrices):
        Cs[i, :n_sources, :n_sources] = np.diag(powers[i])
        Cs[i, n_sources: , n_sources:] = np.diag(sigma_noise * rng.rand(n_channels-n_sources))
    # Generate covariances X:
    if noise_A != 0.:
        X = np.array([a.dot(cs).dot(a.T) for a, cs in zip(A_list, Cs)])
    else:
