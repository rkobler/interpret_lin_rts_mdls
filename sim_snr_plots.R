...
myplot <- df%>%
  ggplot(aes(x=sigma,y=target_score_mu, group=method, color=method)) +
  ...
  labs(y = "normalized mae", x = TeX("$\\sigma$"))+
  ylim(-0.01, 1.1)+
...
myplot <- df%>%
  ggplot(aes(x=sigma,y=pattern_score_mu, group=method, color=method)) +
  labs(title = df_title,subtitle = df_subtitle) +
  ...
  scale_x_log10(breaks =  10^(-10:10),
                minor_breaks = rep(1:9, 21) * (10 ^ rep(-10:10, each=9))) +
  labs(y = TeX("$1 - |a^T \\hat{a}|$"), x = TeX("$\\sigma$")) +
  ylim(-0.01, 0.9)+
  scale_color_manual(values = color_cats, name = NULL) +
  scale_fill_manual(values = color_cats, name = NULL)  +
  geom_hline(yintercept = 1., color = "black", linetype = "dotted",
